...
async function getRecipes() {
  ...
  /**************************/
  let recepies = localStorage.getItem('recipes');
  if (recepies != null) {
    return JSON.parse(recepies);
  }
  // The rest of this method will be concerned with requesting the recipes
  // from the network
  // A2. TODO - Create an empty array to hold the recipes that you will fetch
  let recepies_arr = new Array();
  ...
  return new Promise(async(resolve, reject) => {
    ...
    try{ //A5
      let response_url = await fetch(RECIPE_URLS[i]); //A6

      let response_json = await response_url.json();//A7

      recepies_arr.push(response_json); //A8

      if(recepies_arr.length == RECIPE_URLS.length) { //A9
        saveRecipesToStorage(recepies_arr);
        resolve(recepies_arr);
      }
    } catch(error){ //A10
      console.error(error);
      reject(error); //A11
